...
在14.3.2中，我们看到一种得到**稀疏核机器(sparse kernel machine)** 的方式，即使用一个有核基函数的GLM，加上一个例如$\ell_1$或ARD的稀疏促进先验。另一个方式是将目标函数从**负对数似然(NLL)** 变换到一些其他的损失函数，如我们在6.5.5中讨论的那样。尤其是考虑$\ell_2$正则化的经验损失函数
$$
J(\mathbf{w},\lambda) = \sum_{i=1}^{N}L(y_{i},\hat{y}_{i}) + \lambda \|\mathbf{w}\|^2   \tag{14.45}
$$
其中$\hat{y}_i = \mathbf{w}^{T}\mathbf{x}_i + w_{0}$。(到目前为止，这是在原始的特征空间；我们随后引入核空间)。如果$L$是二次型损失，这等效于岭回归，如果$L$是对数损失函数，这等效于logistics回归。

在岭回归中，我们知道解的形式是$\hat{\mathbf{w}} = (\mathbf{X}^T\mathbf{X} + \lambda I)^{-1}\mathbf{X}^T \mathbf{y}$，且插件预测的形式为$\hat{w}_0 + \hat{\mathbf{w}}^{T}\mathbf{x}$。正如我们在[14.4.3](#1443-核化岭回归kernaliszed-ridge-regression-1433) 节中看的，我们可以将这些方程用一个只包含形式为$\mathbf{x}\mathbf{x}^{\prime}$的内积形式来重写，我们只要将其中的内积用核函数$\kappa(\mathbf{x},\mathbf{x}^{\prime})$来替代即可。这是核化，但不是稀疏的。然而，如果我们将二次型或是对数损失函数用一些其他的函数来代替，我们可以保证解是稀疏的，所以预测只依赖于训练数据的子集，称为**支持向量**。核技巧与一个修改过的损失函数的组合称为**支持向量机或SVM**。这种技术最初是设计用来二分类的，但是后来扩展到回归问题以及多分类问题。

注意到，从概率的角度看的话SVM是不自然的。首先，它们是在损失函数中编码稀疏性，而不是在先验中。其次，它们使用一种算法技巧来编码核，而不是模型明显的一部分。最后，SVMs不会导致概率输出结果，将会导致各种困难，尤其是在多分类的背景下。

...
10章中，我们解释了如果使用图模型来的定义高纬度的联合概率分布。基本思想是在图中添加一条边来建模两个变量之间的相关性。

另一种方法是假设观测变量之间是相关的，因为它们来自一个隐藏的常用“原因”。建模隐变量的模型称为**隐变量模型或LVMs**。正如我们本章中看到的，这种模型比没有隐变量的模型更难拟合。然而，这种模型有两个优势。第一个是LVMs通常比在直接在可见空间内表达相关性的模型含有更少的参数。如果图11.1所示。如果所有节点是二进制的，所有CPD是表格形式，则左侧的模型有17个参数，而右侧模型含有59个自由参数。

![image](Figure11.1.png)

第二，LVM中的隐变量为可以作为一个**bottleneck**，计算数据的压缩形式。这组成了无监督学习的基础。图11.2解释一些泛型的LVM结构，可以用于这个目的。一般有$L$个隐变量$z_{i1},\cdots,z_{iL}$，$D$个可见变量$x_{i1},\cdots,x_{iD}$，通常$D \gggtr L$。如果我们有$L\gt 1$，这里有许多隐因子贡献于每个观测，所以存在多对多的映射。如果$L = 1$，我们只有一个隐变量；这种情况下，通常$z_{i}$是离散的，我们有一对多的映射。也可以有多对一的映射，代表每个观测变量有不同的竞争因子或原因；这种模型组成了概率矩阵因子的基础，如在27.6.2节锁讨论的。最终，我们有一个1对1的映射，可以表示为$\mathbf{z}_i\rightarrow\mathbf{x}_i$。通过允许$\mathbf{z}_i$及/或$\mathbf{x}_i$是向量值，这个表示可以包含其他表示。依赖于似然$p(\mathbf{x}_i|\mathbf{z}_i)$以及先验$p(\mathbf{z}_i)$的形式，我们可以产生大量不同的模型。
![image](Figure11.2.png)

## 11.2 混合模型

LVM最简单的形式是当$z_i\in\{1,\cdots K\}$，表示了一个离散隐状态。我们将为这个使用一个离散先验$p(z_i)=\text{Cat}(\boldsymbol{\pi})$。对于似然，我们使用$p(\mathbf{x}_i|z_i=k)=p_k(\mathbf{x}_{i})$，其中$p_k$是观察的第$k$个基础分布；这个可以是任何类型。总体模型是一个混合模型，因为我们将$K$个基础分布混合在一起如下：
$$
...
\begin{aligned}
    ...
    \text{cov}[\mathbf{x}] &=\sum_{k}\pi_{k}[\mathbf{\Sigma}_k + \boldsymbol{\mu}_k\boldsymbol{\mu}_k^T]-\mathbb{E}[\mathbf{x}]\mathbb{E}[\mathbf{x}]^T     \tag{11.4-11.5}
\end{aligned}
$$
...
### 11.2.3 用于聚类的混合模型(Using mixture models for clustering)
...
在14.7.3节中，我们讲述了如何在**生成分类器**的背景下使用混合模型。我们也可以使用它们来创建用于分类与回归的**判别模型**。例如，考虑图11.6(a)中的数据。因为它们看起来像3个不同的线性回归，每个使用一个不同的输入空间。我们可以允许用混合权重建模，且混合密度是输入相关的：
$$
\begin{aligned}
    p(y_i|\mathbf{x}_i,z_i=k,\boldsymbol{\theta})&=\mathcal{N}(y_i|\mathbf{w}^T_k\mathbf{x}_i,\sigma^2)  \tag{11.8-9} \\
    p(z_i|\mathbf{x}_i,\boldsymbol{\theta}) &= \text{Cat}(z_i|\mathcal{S}(\mathbf{V}^T\mathbf{x}_i)) 
\end{aligned}
$$
看图11.7(a)的。

这个模型称为**专家混合(mixture of experts)**或MoE。思想是每个子模型可以被认为是输入空间内一个特定区域的"专家"。函数$p(z_{i}=k|\mathbf{x}_i,\boldsymbol{\theta})$称为一个**门控函数**，决定使用哪个专家，这依赖于输入值。例如，图11.6(b)显示了如何"刻画"一维输入空间，图11.6(a)单独显示了专家的预测(这种情况下，专家只是线性回归模型)，图11.6(c)显示了模型的总体预测
$$
...
需要清楚的是，我们可以为专家插入任何模型。例如，我们可以使用神经网络来表示门控函数以及专家。得到模型称为一个**混合密度网络**。这样的模型训练起来更慢，但是比混合专家更加灵活。
...
混合专家模型在求解反问题时很有效。这些问题是我们必须要反转多对一映射时的问题。一个典型的例子是在机器人中，其中终端影响器$\mathbf{y}$的定位是由motor$\mathbf{x}$的联合角度唯一确定的。然而，对于任意给定的位置$\mathbf{y}$，存在许多联合$\mathbf{x}$的设定可以产生这个。

## 11.3 混合模型的参数估计(Parameter estimation of mixture models)

我们已经看到给定观测值后如何计算在隐变量之上的后验，但是是假设参数为已知的。本节中，我们讨论如何学习参数。
![image](Figure11.9.png)
在10.4.2节中，我们显示了当我们有完全数据以及一个因子化先验，那么参数之上的后验也是因子化时，使计算更简单。不幸的是，如果我们有隐变量/缺失值，这就不再成立了。可以从图11.9中看到原因。如果$z_i$是可以观测到的，通过d-分割，我们看到$\boldsymbol{\theta}_z \perp \boldsymbol{z}_x | \mathcal{D}$，因此后验是可因子化的。但是在一个LVM中，$z_i$是隐藏的，参数不再是独立的，且后验不再可因子化，使得计算更难。这也使得MAP与ML估计更加复杂化。

### 11.3.1 不可识别性(Unidentifiablity)

对于一个LVM计算$p(\boldsymbol{\theta}|\mathcal{D})$的主要问题是后验可能有多模式。为了看一下为什么，考虑一个GMM。如果$z_i$是全部可以观测到的，会有一个参数的单模式后验
$$
...
不可识别性会引起Beyasian推断的一个问题。例如，假设我们从后验$\boldsymbol{\theta}^{(s)}\sim p(\mathbf{\theta}|\mathcal{D})$做一些采样，然后取平均值，并尝试近似后验均值$\bar{\boldsymbol{\theta}}=\frac{1}{S}\sum_{s=1}^{S}\boldsymbol{\theta}^{(s)}$(这种类型的蒙特卡洛方法在第24章中会详细解释)。如果采样来自不同的模型，平均是没有意义的。然而，注意到平均后验的预测分布$p(\mathbf{x})\approx \frac{1}{S}\sum_{s=1}^{S}p(\mathbf{x}|\boldsymbol{\theta}^{(s)})$是合理的，因为似然函数对参数来自哪个模态是不变的。
